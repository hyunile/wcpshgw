...
public class D50Layout extends ByteBatchLayout {
	private static final Logger logger = LoggerFactory.getLogger(D50Layout.class);

	private static long SEQ_NO_MAX = 5000000000000L;
	private static long SEQ_NO_MIN = 4000000000000L;
	...
	private static int[] HEADER_ITEM_SIZE = { 1, 10, 8, 8, 4, 4, 50, 50, 16, 9, 90 + 1 };
	private static int[] BODY_ITEM_SIZE = { 1, 32, 200, 17 + 1 };
	private static int[] FOOTER_ITEM_SIZE = { 1, 15, 234 + 1 };
	
	public D50Layout(BatchRepository repository) {
		super(Constants.TRANS_CODE_D50, repository, HEADER_ITEM_SIZE, BODY_ITEM_SIZE, FOOTER_ITEM_SIZE, logger);
	}
	...
	protected Map<String, String> parseHeader(byte[] array) {
		...
		try {
			...
			result.put("transCode", Constants.TRANS_CODE_D50);
...
//			result.put("pageId", res.getPageId());
			...
			String[] tokens = result.get("pageId").split("\\|");
			if(tokens.length == 5) {
				result.put("from", tokens[3]);
				result.put("to", tokens[4]);
			}
			
			checkProcessDate(result);
			String reserveTime = res.getReserveTime();
			if(Utils.isEmpty(reserveTime)) {
				reserveTime = options.getReserveTime(Constants.TRANS_CODE_D50);
			}
			
			if(! Utils.isEmpty(reserveTime)) {
				result.put("processDate", Utils.dateToStr(new Date(), Utils.FRMT_DATE));			
	...
	protected long insertItems(BatchRepository repository, List<HashMap<String, Object>> items) {
		return repository.insertD50Items(items);
	}
