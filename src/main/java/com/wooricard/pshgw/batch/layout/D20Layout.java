...
public class D20Layout extends ByteBatchLayout {
	private static final Logger logger = LoggerFactory.getLogger(D20Layout.class);

	private static long SEQ_NO_MAX = 2000000000000L;
	private static long SEQ_NO_MIN = 1000000000000L;
	...
	public D20Layout(BatchRepository repository) {
		super(Constants.TRANS_CODE_D20, repository, HEADER_ITEM_SIZE, BODY_ITEM_SIZE, FOOTER_ITEM_SIZE, logger);
	}
	...
	protected Map<String, String> parseHeader(byte[] array) {
		...
		try {
			...
			result.put("transCode", Constants.TRANS_CODE_D20);
			...
			if(Utils.isEmpty(reserveTime)) {
				reserveTime = options.getReserveTime(Constants.TRANS_CODE_D20);
			}
