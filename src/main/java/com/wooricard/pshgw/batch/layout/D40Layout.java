...
public class D40Layout extends ByteBatchLayout {
	private static final Logger logger = LoggerFactory.getLogger(D40Layout.class);

	private static long SEQ_NO_MAX = 4000000000000L;
	private static long SEQ_NO_MIN = 3000000000000L;
	...
	private static int[] HEADER_ITEM_SIZE = { 1, 8, 10, 50, 182 };
	private static int[] BODY_ITEM_SIZE = { 1, 16, 20, 13, 12, 20, 2, 2, 2, 10, 1, 15, 40, 12, 1, 33, 1, 20, 30 };
	private static int[] FOOTER_ITEM_SIZE = { 1, 15, 235 };
	
	public D40Layout(BatchRepository repository) {
		super(Constants.TRANS_CODE_D40, repository, HEADER_ITEM_SIZE, BODY_ITEM_SIZE, FOOTER_ITEM_SIZE, logger);
	}
	...
	protected Map<String, String> parseHeader(byte[] array) {
		...
		try {
			...
			result.put("transCode", Constants.TRANS_CODE_D40);
			...
			if(Utils.isEmpty(reserveTime)) {
				reserveTime = options.getReserveTime(Constants.TRANS_CODE_D40);
			}
	...
	protected HashMap<String, Object> parseItem(byte[] array) {
		...
		try {
			...
			if(!Utils.isEmpty(mchName) && mchName.length() > Constants.STR_CUT_MCHNAME) {
...
//			result.put("sendTimeLimit", d != null ? d : "");
	...
	protected long insertItems(BatchRepository repository, List<HashMap<String, Object>> items) {
		return repository.insertD40Items(items);
	}
